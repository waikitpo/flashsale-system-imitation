...
```bash
cd backend
# Create data directory (for WAL logs), otherwise it will crash on startup
mkdir -p data 
go mod tidy
go build -o server .
```
...
If you encounter issues running the project for the first time, check the following common causes:

### 1. Crash on Startup
*   **WAL Directory Missing**:
    *   **Symptom**: Program exits immediately, log shows `Panic` or C++ exception.
    *   **Reason**: The `backend/data` directory is used for WAL logs. Since git ignores empty directories, it might be missing after cloning.
    *   **Solution**: Manually create the directory: `mkdir -p backend/data`.
*   **Segmentation Fault (Wild Pointer)**:
    *   **Symptom**: `segmentation fault (core dumped)`.
    *   **Reason**: Some test pointers in the C++ Worker class (e.g., `check_correctness_`) were not initialized to `nullptr` in some compiler environments, leading to random memory access.
    *   **Solution**: Ensure these pointers are explicitly initialized in the `backend/handler/worker.hpp` constructor (Fixed in latest code).

### 2. Dependency Issues
*   **Redis Connection Refused**:
    *   **Symptom**: `dial tcp 127.0.0.1:6380: connect: connection refused`.
    *   **Reason**: Docker container is not running.
    *   **Solution**: Run `docker-compose up -d redis`.

### 3. Database Mode Selection
*   **Default (SQLite)**: If no environment variable is set, the system defaults to embedded SQLite.
*   **Enable PostgreSQL**: Set `USE_PG=true` environment variable to start the service for high-performance full-link stress testing.

### 4. Redundant Directory Issue
*   **backend/handler/db Directory**:
    *   **Symptom**: A `db` folder appears in `backend/handler/` after running `go test` or benchmarks.
    *   **Reason**: Test code uses a relative path (`db/seckill.db`) to initialize SQLite, causing a new database file to be created in the test running directory (`backend/handler`).
    *   **Solution**: This is a temporary file generated by tests and **can be safely deleted or ignored**. The real runtime database is located in `backend/db/`.
