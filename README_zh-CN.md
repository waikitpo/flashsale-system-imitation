...
*   **物理核心 vs 逻辑核心**:
    *   **C++ 核心 (热路径)**: 为库存引擎分配固定数量的 Worker（例如 4 个线程）。这些线程被设计为占用**独立的物理核心**，避免了 SMT（超线程）带来的资源争抢（ALU/FPU/L1 缓存）。
...
为了消除 GC 停顿和分配器开销，实施了严格的 **"循环内无 New (No-New-in-Loop)"** 策略。
...
### 4. 削峰填谷与背压机制 (Traffic Shaping & Backpressure)
系统通过**多级有界队列**实现了天然的削峰填谷能力，防止下游（数据库）被瞬时流量冲垮。

*   **蓄水池效应**: MPMC 队列 (容量 1024*1024) 充当了巨大的缓冲区。当 20k/s 的请求涌入，而数据库只能处理 1k/s 时，多余的请求会暂存在队列中，由 Worker 线程以恒定速率处理。
*   **Fail-Fast (快速失败)**: 系统拒绝无限排队。当 MPMC 队列满时，`Enqueue` 操作会立即返回 `false`，接入层直接向客户端返回 `503 Service Unavailable`。这应该比让用户长时间等待超时要好得多，也防止了系统内存耗尽 (OOM)。
*   **隐式背压传递**:
    1.  **DB 变慢** -> Worker 提交结果阻塞 -> Worker 停止从 SPSC 取任务。
    2.  **SPSC 填满** -> Dispatcher 分发阻塞 -> Dispatcher 停止从 MPMC 取任务。
    3.  **MPMC 填满** -> 接入层无法入队 -> **触发限流**。
    这一链条确保了系统各组件始终在能力范围内工作。

### 5. 拒绝策略与熔断 (Rejection Policy & Circuit Breaking)
当系统过载或组件失效时，采用多层防御策略来保护系统核心：

*   **Redis 熔断器**: 当 Redis 连续失败 5 次，系统进入**熔断状态**，直接返回 `503 System Busy`，等待 5 秒后尝试恢复。这防止了存储层故障拖垮整个服务。
*   **队列满拒绝 (Load Shedding)**: 当 MPMC 队列满时，HTTP 层直接返回 `503 Queue full` 并触发 Redis 回滚。这是最底层的过载保护，防止内存溢出。
*   **重复请求拦截**: 利用 Redis 原子操作或 C++ 内存去重，对重复 `request_id` 返回 `409 Duplicate purchase`。
*   **库存耗尽拦截**:
    1.  **L1 (Redis)**: `409 Sold out` (分布式拦截，大多数流量在此停止)。
    2.  **L2 (C++ Local)**: `409 Sold out` (本地原子计数器拦截，作为最后一道防线)。
...
### Nginx 与多节点分流测试
在测试环境中引入 Nginx，核心目的是为了验证**多机部署 (Multi-Node)** 场景下的流量分发与系统行为。
...
├── backend/
│   ├── handler/         # 核心逻辑
│   │   ├── bridge.cpp   # C++ 桥接层 (CGO)
│   │   ├── worker.hpp   # C++ 工作线程实现 (Thread-per-Core)
│   │   ├── mpmc_queue.hpp # Vyukov 无锁队列实现
